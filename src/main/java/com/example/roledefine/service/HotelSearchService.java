...
public class HotelSearchService {
    ...
    public Mono<String> confirmBooking(ConfirmBookingRequestDTO confirmRequest) {
        String paymentIntentId = confirmRequest.getPaymentIntentId();
        log.info("Confirming booking for paymentIntentId: {}", paymentIntentId);

        HotelTransaction transaction = hotelTransactionRepository.findByPaymentIntentId(paymentIntentId)
                .orElseThrow(() -> new RuntimeException("Transaction not found for payment ID: " + paymentIntentId));

        log.info("Payment verification successful (simulated).");
        transaction.setBookingStatus("PENDING_CONFIRMATION");
        hotelTransactionRepository.save(transaction);

        return Mono.fromCallable(() -> {
                    HotelBookingRequestDTO trawexRequest = objectMapper.readValue(transaction.getBookingRequestData(), HotelBookingRequestDTO.class);

                    trawexRequest.setClientRef(UUID.randomUUID().toString());

                    credentialProvider.populate(trawexRequest);
                    return trawexRequest;
                })
                .flatMap(trawexRequest ->
                        webClient.post()
                                .uri("/hotel_trawexv6/hotel_book")
                                .bodyValue(trawexRequest)
                                .retrieve()
                                .bodyToMono(String.class)
                )
                .flatMap(trawexResponse -> {
                    try {
                        JsonNode responseNode = objectMapper.readTree(trawexResponse);
                        JsonNode statusNode = responseNode.path("status");
                        if ("CONFIRMED".equalsIgnoreCase(statusNode.asText())) {
                            String refNum = responseNode.path("referenceNum").asText();
                            String confNum = responseNode.path("supplierConfirmationNum").asText();
                            transaction.setBookingStatus("CONFIRMED");
                            transaction.setReferenceNum(refNum);
                            transaction.setSupplierConfirmationNum(confNum);
                            hotelTransactionRepository.save(transaction);
                            return Mono.just(trawexResponse);
                        } else {
                            log.error("Trawex booking failed. Full response: {}", trawexResponse);
                            transaction.setBookingStatus("FAILED");
                            hotelTransactionRepository.save(transaction);
                            return Mono.error(new RuntimeException("Booking failed at supplier. Full response: " + trawexResponse));
                        }
                    } catch (Exception e) {
                        return Mono.error(new RuntimeException("Error parsing Trawex confirmation response.", e));
                    }
                });
    }
}